...
import { STAGE_COUNT, useExperience } from '../state/experience'
import { story } from '../data/story'

const WHEEL_COOLDOWN_MS = 1250
const WHEEL_THRESHOLD_PX = 70
const SWIPE_THRESHOLD_PX = 64
const STAGE_LAST = STAGE_COUNT - 1
...
export function useExperienceNav(): void {
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent): void => {
      ...
      // If wheel originates inside the memories scroll container that can still scroll in that direction, don't navigate stages
      const target = e.target as HTMLElement | null
      const scrollEl = target?.closest?.('[data-memories-scroll]') as HTMLElement | null
      if (scrollEl) {
    ...
    const onTouchStart = (e: TouchEvent): void => {
      ...
      // If touch starts inside memories scroll area that is scrollable, don't treat as stage swipe unless at boundary
      const target = e.target as HTMLElement | null
      const scrollEl = target?.closest?.('[data-memories-scroll]') as HTMLElement | null
      if (scrollEl) {
        // Let native scroll handle it — mark as not tracking for stage nav
