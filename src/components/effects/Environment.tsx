...
export function Environment({
  ...
  return (
    <>
      ...
      {/* particles — single declaration per preset, soft & sparse */}
      {showParticles && preset === 'dream' && (
        <>
          <ParticleField preset="romantic" count={52} scale={[14, 6, 14]} opacity={0.32} />
          <RomanticOrbs count={3} radius={6} height={2.8} />
        </>
      )}
      {showParticles && preset === 'garden' && (
        <>
          <ParticleField preset="romantic" count={42} scale={[16, 7, 16]} color="#FFD6A5" opacity={0.28} />
          <RomanticOrbs count={3} radius={5.5} height={2.8} />
        </>
      )}
      {showParticles && preset === 'night' && <ParticleField preset="dust" color="#FFE8C7" count={48} opacity={0.26} />}
      {showParticles && preset === 'dawn' && <ParticleField preset="dust" count={42} scale={[14, 6, 16]} color="#FFD6A5" opacity={0.28} />}
      {showParticles && preset === 'sunset' && <ParticleField preset="dust" count={36} scale={[18, 7, 18]} color="#FFD6A5" opacity={0.24} />}
