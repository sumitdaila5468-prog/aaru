  ...
  return (
    <section id="final" className="relative overflow-hidden bg-[#0F040A] px-4 py-14 md:px-6 md:py-20">
      ...
      <div className="mx-auto flex max-w-5xl flex-col items-center">
        ...
        >
          ...
          <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            ...
            <button onClick={()=>setShowLast(true)} className="w-full rounded-full border border-white/10 bg-white/[0.04] px-6 py-3 text-[11px] tracking-[0.16em] uppercase text-white/70 backdrop-blur hover:bg-[#0F040A] sm:w-auto">💌 READ ONE LAST MESSAGE</button>
          </div>
      ...
      <Modal open={showLast} onClose={()=>setShowLast(false)} title={finalContent.lastMessageTitle} maxWidth="max-w-lg">
        <p className="whitespace-pre-line font-display text-[14px] italic leading-relaxed text-[#FFE8EE]/78">{finalContent.lastMessage}</p>
        <div className="mt-6 flex gap-2">
          ...
          <button onClick={()=>{ setShowLast(false); const el=document.getElementById('home'); el?.scrollIntoView({behavior:'smooth'}) }} className="rounded-full border border-[#EFA7B8]/20 bg-white/[0.04] px-6 py-2 text-[11px] tracking-[0.12em] uppercase text-white/60">back to top ↑</button>
        </div>
      </Modal>
    </section>
  )
